...
static uint32_t
getUleb128 (const uint8_t *in, size_t *num_bytes)
{
  uint64_t val        = 0;
  uint32_t i          = 0, more;
  size_t   bytes_read = 0;
...
ESEAnnexBStream::processToNextFrame ()
{
  ...
  size_t fileSize = m_reader->streamSize ();

  // HACKS: For quickly testing the approach, OK for CTS, long sequences are not practical.
  assert (fileSize < (5 * 1024 * 1024));
  if (m_buffer.size () == 0)
    m_buffer = m_reader->getBuffer (fileSize);
  ...
  ptr += frameUlebSize;
  size_t frameStartOffset = ptr - m_buffer.data();
  size_t frameEndOffset = frameStartOffset + frameSize;
